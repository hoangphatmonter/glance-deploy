...
try {
    isMRECEnabledOnLP = location.search
        .substring(1)
        .split('&')
        .find((a) => {
            return a.startsWith('LPMREC')
        })
        .split('=')[1]
} catch (error) {
...
function showBumperAd() {
    if (isMRECEnabledOnLP === 'true') {
        window.LPBannerInstance = window?.GlanceGamingAdInterface?.showLoadingPageBannerAd(
            window.LPMercObj,
        ...
        if (isBumperAd) {
            ...
            isAdLoaded = window.BumperAd.isAdLoaded()
            sendCustomAnalyticsEvent('Game_loading_screen_start', {})
            isNormalGame = false
            window.BumperAd.showAd()
        } else {
            window.LPBannerInstance = window?.GlanceGamingAdInterface?.showLoadingPageBannerAd(
                window.LPMercObj,
                window.bannerCallbacks
        ...
        <div id="replayGame"></div>
          <div id="loaderPage">
              <div id = "loaderPageContent">
                  <img src="./assets/glance/loadingText.png" />

                  <div class="progress-bar">
                      <div class="progress-barnew">
                          <img id = "progressBarImg" src="./assets/glance/loader.png" />
                      </div>
                  </div>
              </div>
              <div id="div-gpt-ad-2">
