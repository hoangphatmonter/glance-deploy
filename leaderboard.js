	...
	<div class='gameoverPage'>
		...
		<div class = "lb-footer">
			<div class = "lb-footer-content">
				<p>Powered by&nbsp;</p><img class="glance_logo" src = "assets/glance/glance_logo.png">
			</div>
...
function showLeaderBoard(score) {
    if (recUI === 'true') {
        ...
        if (window.GlanceGamingAdInterface) {
            window.LBBannerInstance = window.GlanceGamingAdInterface.showBannerAd(
                LBBannerObj,
                bannerCallbacks
            )
        }
...
function populateLeaderBoard(msg) {
    if (msg == 'success') {
        getLeaderBoardData(function (res) {
            // ? Some times the response is not an object
            if (typeof res !== 'object') {
                // make a default response
                res = {
                    user_current_rank: 0,
                    current_user_max_score: {
                        scores__max: 0,
                    },
                    score_details: [],
                }
            ...
            let bestScore = 0
            if (res.current_user_max_score) {
                bestScore = res.current_user_max_score.scores__max
            }

            let scoreDetails = []
            if (res.score_details) {
                scoreDetails = res.score_details
            }

            $('#bestScore').html(`Best : ${bestScore}`)
            let logoSrc = 'user-icon.png'
            let topScorer
            if (scoreDetails.length > 0) {
                scoreDetails.map((items, i) => {
                    if (i == 0) {
                ...
                let yourScoreChars = bestScore.toString().length
                ...
                let yourRow =
                    ...
                    `" alt="" class="lb-icon">
                                    ...
                                    <p class="lb-gamepoints">${bestScore}</p>
                                    </div>
                                </div>`
            ...
            $('#yourRank').html(`Rank : ${res.user_current_rank || 'NA'}`)
        })
    } else {
